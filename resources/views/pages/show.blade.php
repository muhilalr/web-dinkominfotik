  ...
  <section class="bg-utama relative overflow-hidden pb-12 pt-28 text-white lg:pb-16 lg:pt-36">
    ...
    <div class="container relative z-10 mx-auto px-4">
      ...
      <nav
        ...
        <span class="shrink-0 text-blue-200/80">{{ $category ?? 'Halaman' }}</span>
        <span class="text-blue-200/50">&sol;</span>
        <span class="truncate font-semibold text-white">{{ $page->title ?? ($title ?? 'Judul Halaman') }}</span>
      </nav>
      ...
      <div class="max-w-4xl">
        ...
        @if (isset($page->updated_at) || isset($updatedAt))
          <div class="flex items-center gap-4 border-t border-white/10 pt-2 text-xs text-blue-100/90">
            <span class="flex items-center gap-1.5">
              ...
              Diperbarui:
              {{ isset($page) ? $page->updated_at->translatedFormat('d F Y') : $updatedAt ?? now()->translatedFormat('d F Y') }}
            </span>
            <span class="flex items-center gap-1.5">
              <svg class="h-4 w-4 text-blue-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <main class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm md:p-10 lg:col-span-2">
        @if (isset($page->featured_image) || isset($featuredImage))
          <div class="mb-8 overflow-hidden rounded-xl border border-slate-100 shadow-sm">
            <img src="{{ $page->featured_image ?? $featuredImage }}"
              alt="{{ $page->title ?? ($title ?? 'Featured Image') }}" class="max-h-105 h-auto w-full object-cover" />
          </div>
        @endif
          ...
          @else
            ...
            <div class="space-y-6">
              <p class="text-base leading-relaxed text-slate-700">
                Halaman ini menyajikan informasi resmi terkait Dinas Komunikasi, Informatika dan Statistik Kabupaten
                Bangka. Kami berkomitmen menyediakan tata kelola pemerintahan yang transparan dan akuntabel.
              </p>

              <h2 class="mt-6 border-b border-slate-200 pb-2 text-xl font-bold text-slate-800">
                Vision & Mission Statement
              </h2>

              <div class="border-utama my-6 rounded-r-xl border-l-4 bg-blue-50/50 p-5">
                <h3 class="text-utama mb-2 text-base font-bold">Visi Kabupaten Bangka</h3>
                <p class="text-sm italic text-slate-700 md:text-base">
                  &ldquo;Terwujudnya Kabupaten Bangka yang Sejahtera, Maju, dan Berkelanjutan Melalui Tata Kelola
                  Pemerintahan Digital.&rdquo;
                </p>
              </div>

              <h3 class="mb-3 mt-6 text-lg font-bold text-slate-800">Misi Utama:</h3>
              ...
              <div class="rounded-xl border border-slate-200/80 bg-slate-50 p-5">
                <h4 class="mb-3 flex items-center gap-2 text-sm font-bold text-slate-800">
                ...
                <div class="flex items-center justify-between rounded-lg border border-slate-200 bg-white p-3 text-xs">
                  <span class="truncate pr-2 font-medium text-slate-700">Dokumen_Resmi_Informasi_Publik.pdf</span>
                  <a href="#"
                    class="bg-utama inline-flex shrink-0 items-center gap-1 rounded px-3 py-1.5 font-semibold text-white transition-colors hover:bg-blue-900">
                    Unduh
