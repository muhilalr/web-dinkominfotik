<x-app-layout>
  ...
  <section class="bg-utama relative overflow-hidden pb-12 pt-28 text-white lg:pb-16 lg:pt-36">
    ...
    <div class="container relative z-10 mx-auto px-4 md:px-12">
      ...
      <div class="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
        <div class="max-w-2xl">
          <div class="mb-3 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold backdrop-blur-xs">
            <span class="h-2 w-2 rounded-full bg-emerald-400"></span>
            Pusat Dokumen & Informasi
          </div>
        ...
        <div class="w-full max-w-md">
          <form action="{{ route('bank-data.index') }}" method="GET" class="relative">
            @if ($currentTahun)
  ...
  <section class="container mx-auto px-4 py-8 lg:py-12">
    @if ($search)
      <div class="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-blue-100 bg-blue-50/60 p-4 text-sm text-slate-700">
        <div>
          Hasil pencarian untuk: <strong class="text-utama font-bold">"{{ $search }}"</strong>
          <span class="text-xs text-slate-500">({{ $bankDatas->total() }} dokumen ditemukan)</span>
        </div>
        <a href="{{ request()->fullUrlWithQuery(['q' => null]) }}" class="text-xs font-bold text-red-600 hover:underline">
    ...
    @if ($availableYears->count())
      <div class="mb-8 flex flex-wrap items-center gap-2">
        <a href="{{ route('bank-data.index') }}"
          class="{{ empty($currentTahun) ? 'bg-utama text-white shadow-xs' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50' }} rounded-xl px-4 py-2 text-xs font-semibold transition-all">
          Semua Tahun
        </a>
        @foreach ($availableYears as $year)
          <a href="{{ request()->fullUrlWithQuery(['tahun' => $year]) }}"
            class="{{ $currentTahun === $year ? 'bg-utama text-white shadow-xs' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50' }} rounded-xl px-4 py-2 text-xs font-semibold transition-all">
            {{ $year }}
          </a>
        @endforeach
      </div>
